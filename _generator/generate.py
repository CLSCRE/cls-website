...
def main():
    ...
    # and others). Included so full regens preserve them.
    for _lp_html in sorted((WEBSITE_DIR / "landing").glob("*.html")):
        # Landing pages are noindexed intentionally for ad tests; only
        # include city-specific commercial-mortgage LPs which ARE indexed.
        if "commercial-mortgage" in _lp_html.name or "-bridge-loans" in _lp_html.name or "-construction-loans" in _lp_html.name or "-multifamily-loans" in _lp_html.name or "-sba-commercial-loans" in _lp_html.name or "-commercial-real-estate-loans" in _lp_html.name:
            sitemap_urls.append({
